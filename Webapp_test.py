...
xgb = joblib.load("pima.joblib.dat")
...
st.markdown("Ist Ihre URL gefährlich oder können Sie sie sicher aufrufen? Lassen Sie ihre URL in 5 Kategorien klassifizieren!")
...
with st.form("URL"):
        url = st.text_input("URL hier einfügen:")
        ...
        ############################################
            ...
            for x in [Klasse]:
                        ...
                        if x == 4:
                                st.error("Es handelt sich mit %i Prozent Sicherheit um Spam!" % (Probability[:,4]*100))
                                st.write("Was ist Spam?")
                                st.caption("Was ist Spam? Es ist fast unmöglich unerwünschte Massennachrichten, welche hauptsächlich zu Werbezwecken verschickt werden, aus dem Alltag wegzudenken. Derartige Junk-E-Mails sind zwar lästig, hinter ihnen stecken aber in vielen Fällen nur unternehmerische Interessen und enthalten keine Schadsoftware. Dies ist auch der entscheidende Unterschied zu Phishing Links, bei welchen betrügerische Absichten gefährliche Konsequenzen für den Betroffenen haben können. Der Spammer hingegen macht Gebrauch von einer kostengünstigen Methode, ein Produkt grossflächig zu vermarkten. ")
                                st.write("Wie kann ich Spam identifizieren und wie gehe ich dagegen vor?")
                                st.caption("Spam ist oft unvermeidbar, wenn Unternehmen Kontaktinformationen an Dritte weitergeben und keine eigenhändige Anmeldung zu einem Newsletter erfolgt. Dennoch verfügen die meisten E-Mail-Clients über eine eingebaute Funktion zur Klassifizierung von Spam, wobei man zur Performancemaximierung auch auf falschklassifizierten Spam hinweisen sollte. Obwohl es sich oft nur um Werbung handelt, sollte man dennoch nicht direkt auf die Links klicken, sondern sie einfach ignorieren.")
                                st.caption("Quelle: https://www.avast.com/de-de/c-spam")
            
            st.write("Disclaimer")                    
            st.caption("Wir möchten maximale Sicherheit und Vorsicht im Umgang mit Cybersicherheit fördern. Aus diesem Grund haben wir bei der Programmierung und Auswahl unserer Modelle stets die Präzision maximiert. Das heisst, dass wir Links eher als z.B. Spam einstufen, obwohl sie Benign sind, anstatt zu risikieren, dass ein als Benign klassifzierter Link eigentlich Spam ist. Nichtsdestotrotz übernehmen wir keine Haftung für allfällige Schäden, die aufgrund eines von URLegit falsch klassifizierten Links entstehen.")
        ...
        ############################################
            ...
            with st.expander("Sehen Sie sich hier die Feature Importance an"):
                Importance = np.array(xgb.feature_importances_)
...
with st.expander("Öffnen um zu den Visualisierungen zu gelangen"):
       ...
       with st.form("Visualisierung"):
           names = Data.columns.drop(["label", "Kategorie", "Unnamed: 0"])
